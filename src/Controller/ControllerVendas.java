...
public class ControllerVendas implements Initializable{
	...
	public void gerarPdf(List <VendasVO> vendas){
		...
		try {
			...
			par = new Paragraph(" ");
			doc.add(par);
			Paragraph tex = new Paragraph("Código da venda: " + ((VendasVO) vendas).getCodigo());
			doc.add(tex);
			tex = new Paragraph("Produtos :" + ((VendasVO) vendas).getProdutos());
			doc.add(tex);
			tex = new Paragraph("Quantidade :" + ((VendasVO) vendas).getQuantidade());
			doc.add(tex);
			tex = new Paragraph("Valor(R$) :" + ((VendasVO) vendas).getValor());
			doc.add(tex);
			doc.add(par);
